// Web: starts the players' web server (skaldbok_web) together with the app, shows whether it is up and at which address,
// and lists each player's personal link. The server is a separate program that reads the same files as the app; this module only
// launches it, watches it and stops it when the app closes, so opening the app is enough for players to reach their pages.
#ifdef _WIN32
#define NOMINMAX
...
public:
    explicit WebModule(Host& host) : Module(host) {
        ...
        // opening the app is all it takes: the server starts with it, unless it was switched off (or --no-web, for tests)
        const char* off = SDL_getenv("SKALDBOK_NO_WEB");
        if (host_.settings().web.autostart && moduleIsOn(host_, *this) && !(off && *off == '1')) start();
    }
    ...
    const char* summary() const override { return "Starts the players' web server with the app, so each player can open their character, their party and the rules in a browser."; }
    Layout layout() const override { return Layout::Full; }
    bool showInNav() const override { return false; }             // it lives in the Settings menu (the gear)
    const char* hostedBy() const override { return "settings"; }
    bool wantsRedraw() const override { return proc_ && SDL_GetTicks() < watchUntil_; }   // while it is starting, keep looking

    void enabledChanged(bool on) override {
        if (on && host_.settings().web.autostart) start();
        if (!on) stop();
    }
    ...
    void drawFull() override {
        ...
        bigText("Web for players", 1.6f, kAccent);
        ImGui::TextWrapped("Each player opens their own link in a browser (a phone is fine) and sees their character, their party and the rules. "
                           "The page updates by itself when you change something here. The server starts with the app.");
        ...
        if (!proc_) {
            if (ImGui::Button("Start the web server")) start();
        } else {
        ...
        ImGui::Spacing();
        ImGui::Separator();
        if (ImGui::Checkbox("Start the web server when the app opens", &w.autostart)) host_.settings().save();
