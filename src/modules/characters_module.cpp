...
public:
    ...
    const char* summary() const override { return "The players' character sheets and a creator that follows the Rulebook. Saved as JSON files."; }
...
private:
    ...
    void drawWizard() {
        ...
        if (content.entries(Kind::Kin).empty() || content.entries(Kind::Profession).empty()) {
            ImGui::TextWrapped("There is no kin or profession loaded, so a character cannot be created. Check Settings > Content packs.");
