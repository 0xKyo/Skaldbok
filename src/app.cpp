...
void App::showModule(const std::string& id) {
    ...
    if (!m || !isOn(*m)) return;
    // A module that lives inside another one (Web inside Settings) opens as that module's page, on its tab.
    Module* target = m;
    if (*m->hostedBy()) {
        Module* host = moduleById(m->hostedBy());
        if (host && isOn(*host)) {
            target = host;
            host->showChild(id);
        }
    ...
    if (!*target->hostedBy() && std::string(target->id()) == "settings") beforeSettings_ = active_;   // the gear goes back to this
    active_ = target;
}

// The gear: opens Settings, and closes it again (back to what was open before).
void App::toggleSettings() {
    if (active_ && std::string(active_->id()) == "settings") {
        if (beforeSettings_ && isOn(*beforeSettings_) && beforeSettings_ != active_) active_ = beforeSettings_;
...
bool App::wantsRedraw() const {
    ...
    for (const auto& m : modules_)
        if (isOn(*m) && m->wantsRedraw()) return true;
    return false;
}
...
void App::applyStartup(const StartupOptions& o) {
    ...
    if (!o.select.empty()) {
        ...
        Selection found;
        bool done = false;
        if (active_ && isOn(*active_) && active_->findByName(want, found)) done = true;   // --tab narrows the choice
        for (const auto& m : modules_) {
...
// The settings gear, at the right end of the top bar: grey at rest, teal on hover and while Settings is open. The icon is a PNG
...
void App::drawGearButton() {
    ...
    const bool open = active_ && std::string(active_->id()) == "settings";
    ImGui::PushID("gear");
    ...
    if (clicked) toggleSettings();
}

void App::drawTopBar() {
    ImGui::BeginDisabled(historyPos_ <= 0);
    if (ImGui::Button("<")) goBack();
    ImGui::EndDisabled();
    if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) ImGui::SetTooltip("Back (Alt+Left)");
    ImGui::SameLine(0, 4);
    ImGui::BeginDisabled(historyPos_ + 1 >= static_cast<int>(history_.size()));
    ...
    if (search) {
        ...
        if (ImGui::InputTextWithHint("##search", "Search everything… (Ctrl+K)", search->queryBuffer(), search->queryCapacity())) {
            search->runQuery();
...
void App::drawNav() {
    ...
    for (const auto& mp : modules_) {
        Module& m = *mp;
        if (!isOn(m) || !m.showInNav()) continue;
        ...
        ImGui::PushID(m.id());
        if (ImGui::Selectable("##nav", active_ == &m, 0, ImVec2(0, lineH() + 12))) {
            active_ = &m;
            if (std::string(m.id()) == "search") focusSearch_ = true;
        }
...
void App::drawWelcome() {
    ...
    ImGui::BulletText("Up / Down   move through a list after clicking it");
...
void App::frame(float width, float height) {
    ...
    // a module that was switched off cannot stay in front
    if (!active_ || !isOn(*active_)) showModule("search");
    ...
    if (m->layout() == Layout::Full) {
        // the whole width: a mosaic of cards, the settings
