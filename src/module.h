...
//
// A module is one self-contained part of the app (Encounter, Characters, Messages, the spell mosaic...). It draws
...
public:
    ...
    virtual const char* summary() const = 0;             // one line in Settings > Modules
    virtual const char* group() const { return ""; }     // navigation heading; modules with the same group sit together
    virtual bool required() const { return false; }      // cannot be switched off (Search, Settings)
    virtual bool showInNav() const { return true; }      // false: no sidebar entry (Settings, and the modules it hosts)
    virtual const char* hostedBy() const { return ""; }  // the id of a module whose page contains this one's ("settings")
    virtual void showChild(const std::string&) {}        // a hosting module: bring the page of a hosted module to the front
    ...
    virtual void update() {}                             // once per frame while the module is on (network, timers)
    virtual bool wantsRedraw() const { return false; }   // something is animating or in flight
    virtual void enabledChanged(bool) {}                 // switched on or off in Settings
...
inline bool moduleIsOn(Host& host, const Module& m) { return m.required() || host.settings().moduleEnabled(m.id()); }

// The enabled module offering service T, or nullptr.
template <class T>
T* serviceOf(Host& host) {
    for (const auto& m : host.modules())
        if (moduleIsOn(host, *m))
